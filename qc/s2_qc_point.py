...
Usage:
    python qc/s2_qc_point.py --run artifacts/runs/20260131_154944_S2_b90.csv
...
def main() -> int:
    ...
    if report_dir is not None:
        ...
        if bs_csv.exists():
            ...
            def f(key: str) -> float | None:
                ...
                if s is None:
                    return None
                # Pandas may keep NaN as float even after astype(str) on some versions.
    ...
    pf_state = None
    if pf_discharge_mah is not None:
        if pf_discharge_mah > 0:
            pf_state = "discharging"
        elif pf_discharge_mah < 0:
            pf_state = "charging"
        else:
            pf_state = "flat"
    ...
    # Prefer perfetto only when it looks like a valid discharge run.
    if (pf_discharge_mah is not None and pf_discharge_mah > 0) or (pf_state is None and pf_power_mw_mean is not None):
