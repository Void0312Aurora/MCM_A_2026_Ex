...
* 第2步：确定观测量与采样协议（数据支持而非替代）
  * 产出：采样字段清单（SOC%、电压、温度、charge counter、亮度、CPU 频点驻留等）、采样频率（1–5s）、场景切换记录方式；实验场景集合以 [docs/experiment_matrix.zh-CN.md](docs/experiment_matrix.zh-CN.md) 为准。
...
* 第5步：参数估计与验证 + Time-to-Empty（TTE）预测
  * 产出：用最小实验集合（S0/S1/S2/S3/S4 + V1）估计 Pbase**P**base、屏幕功耗曲线、CPU功耗缩放系数等；对多个使用情景计算/模拟 t∗**t**∗（空电时间），并与观测/合理行为对比。
